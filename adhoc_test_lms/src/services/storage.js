...
export const StorageService = {
  ...
  _mapCourse: (course) => {
    ...
    const p1 =
      parseFloat(course.price_1month) || parseFloat(prices["1month"]) || 0;
    const p3 =
      parseFloat(course.price_3months) || parseFloat(prices["3months"]) || 0;
    const p6 =
      parseFloat(course.price_6months) || parseFloat(prices["6months"]) || 0;
  ...
  getCourses: async (forceRefresh = false) => {
    try {
      ...
      const mappedCourses = raw.map((course) =>
        StorageService._mapCourse(course),
      );
  ...
  enroll: async (courseId, plan = '3months', coinsUsed = 0) => {
        ...
        try {
          ...
          if (!window.Razorpay) {
            ...
            if (!loaded) {
              return resolve({
                success: false,
                message: "Payment system failed to load",
              });
            }
          }
