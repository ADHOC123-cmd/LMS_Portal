...
export const api = {
  ...
  progress: {
    ...
    markComplete: (lessonId, token) =>
      request(`/progress/lesson/${lessonId}/complete`, {
        method: "POST",
        token,
      }),
  },
  ...
  quizzes: {
    ...
    getMyAttempts: (token) => request("/quizzes/attempts/my", { token }),
  },
  ...
  tickets: {
    ...
    getMy: (token) => request("/tickets/my", { token }),

    getById: (ticketId, token) => request(`/tickets/my/${ticketId}`, { token }),
    ...
    respond: (ticketId, data, token) =>
      request(`/tickets/${ticketId}/respond`, {
        method: "PUT",
        token,
        body: data,
      }),
    ...
    getStats: (token) => request("/tickets/stats", { token }),
  },
  ...
  feedbacks: {
    ...
    getAll: (token) => request("/feedbacks", { token }),

    updateDisplay: (id, showOnHome, token) =>
      request(`/feedbacks/${id}/display`, {
        method: "PUT",
        token,
        body: { showOnHome },
      }),

    getHome: () => request("/feedbacks/home"),
