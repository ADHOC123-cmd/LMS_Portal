      ...
      <div className="max-w-7xl mx-auto">
        ...
        <section className="mb-12 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
          <div>
            ...
            <p className="text-on-surface-variant text-lg font-medium opacity-60 mt-2">
              Overseeing global operational metrics and curriculum health.
            </p>
          </div>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {metrics.map((m, i) => (
            <div
              key={m.label}
              className="bg-surface-container-lowest p-8 rounded-[2.5rem] border border-surface-dim/20 shadow-xl shadow-primary/5 group hover:border-primary/20 transition-all"
            >
              <div className="flex justify-between items-start mb-6">
                <div
                  className={`w-12 h-12 rounded-2xl ${m.color === "primary" ? "bg-primary-fixed text-primary" : "bg-surface-container-high text-secondary"} flex items-center justify-center group-hover:scale-110 transition-transform`}
                >
              ...
              <div className="flex-grow flex items-end justify-between h-64 border-b-2 border-dashed border-surface-dim/30 pb-4 relative z-10 px-4">
                {chartData && chartData.length > 0 ? (
                  chartData.map((data, index) => {
                    const maxRevenue =
                      Math.max(...chartData.map((d) => d.revenue)) || 1000;
                    const heightPercent = Math.max(
                      (data.revenue / maxRevenue) * 100,
                      5,
                    );
                    const widthClass =
                      activeRange === "Yearly" ? "w-1/4" : "w-1/12";
                    return (
                      <div
                        key={index}
                        className={`flex flex-col items-center gap-2 group h-full justify-end ${widthClass}`}
                      >
                        <div
                        ...
                        <span className="text-[10px] font-bold text-secondary uppercase whitespace-nowrap">
                          {data.label}
                        </span>
                      </div>
                    );
                  ...
                  <div className="w-full text-center opacity-30 group h-full flex flex-col justify-center">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest">
                      Real-time Visualizer Initializing...
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex justify-between items-center mb-8">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="pl-11 pr-6 py-3 bg-surface-container-low border-none rounded-xl text-xs font-medium focus:ring-2 focus:ring-primary/10 transition-all w-64"
                  />
                </div>
              </div>
              <div className="space-y-4">
                ...
                analytics.popularCourses.length > 0 ? (
                  analytics.popularCourses.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-6 bg-surface-container-low/50 rounded-2xl hover:bg-surface-container-low transition-all group"
                    >
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="bg-primary-container p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
              <div className="relative z-10">
                <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-[0.4em] mb-4">
                  Tactical Status
                </h4>
                <p className="text-5xl font-headline font-extrabold italic tracking-tighter mb-4 leading-none">
                  99.8%
                </p>
                <p className="text-sm font-medium opacity-60 leading-relaxed mb-8">
                  Overall curriculum health index across all operational
                  sectors.
                </p>
                ...
                <button className="w-full py-4 bg-white/10 backdrop-blur-md rounded-2xl font-bold text-sm text-white hover:bg-white/20 transition-all uppercase tracking-widest">
                  Generate Intel
                </button>
              </div>
            ...
            <div className="bg-surface-container-lowest p-10 rounded-[3rem] border border-surface-dim/20 shadow-xl shadow-primary/5">
              <h4 className="text-xs font-bold text-primary uppercase tracking-widest mb-8 italic">
                Recent Logs
              </h4>
              <div className="space-y-6">
                ...
                analytics.popularCourses.length > 0
                  ? analytics.popularCourses.slice(0, 3).map((item, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-start border-l-2 border-primary/20 pl-4 py-1"
                      >
                        <div>
                        ...
                        <span className="text-[10px] font-medium opacity-40 italic">
                          Recent
                        </span>
                      </div>
                    ))
                  : [
                      {
                        label: "Asset Node",
                        detail: "Cloud Sec v3 deployed",
                        time: "12m ago",
                      },
                      {
                        label: "Security",
                        detail: "Admin login detected",
                        time: "45m ago",
                      },
                      {
                        label: "Intel",
                        detail: "Revenue protocol run",
                        time: "1h ago",
                      },
                    ].map((log, i) => (
                      <div
