...
function AdminBlogsContent() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blogs/admin/all`, {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const url = currentBlog.id 
        ? `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/blogs/${currentBlog.id}`
